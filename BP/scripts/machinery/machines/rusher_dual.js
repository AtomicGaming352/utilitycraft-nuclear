...
const CUSTOM_MACHINE_RECIPES_PROP = "utilitycraft:machine_recipes_data";
const RESERVED_MACHINE_RECIPE_KEYS = new Set([
    "type",
    "recipe_type",
    "entity",
    "machine",
...
function extractCustomRecipeTable(settings) {
    if (!settings || typeof settings !== "object" || Array.isArray(settings)) return null;
    if (settings.recipes && typeof settings.recipes === "object" && !Array.isArray(settings.recipes)) {
        const nested = settings.recipes;
        const nestedKeys = Object.keys(nested);
        if (nestedKeys.length > 0) return nested;
    }
    const table = {};
    let found = false;
    for (const [key, value] of Object.entries(settings)) {
        if (RESERVED_MACHINE_RECIPE_KEYS.has(key)) continue;
...
function loadCustomRecipeTable(entity) {
    const raw = entity?.getDynamicProperty?.(CUSTOM_MACHINE_RECIPES_PROP);
...
function storeCustomRecipeTable(entity, settings) {
    const table = extractCustomRecipeTable(settings);
    if (!table || !entity) return false;
    try {
        entity.setDynamicProperty(CUSTOM_MACHINE_RECIPES_PROP, JSON.stringify(table));
        return true;
    } catch {
        return false;
    }
    ...
    if (Array.isArray(recipe?.outputs) && recipe.outputs.length > 0) {
        return recipe.outputs
            .map((output, index) => ({
                ...
                chance: typeof output.chance === 'number' ? Math.max(0, Math.min(1, output.chance)) : (typeof output.output_chance === 'number' ? Math.max(0, Math.min(1, output.output_chance)) : 1),
            }))
    ...
    if (typeof recipe?.output === 'string' && recipe.output.length > 0) {
        outputs.push({
            ...
            chance: typeof recipe.primary_chance === 'number'
                ...
                : (typeof recipe.output_chance === 'number' ? Math.max(0, Math.min(1, recipe.output_chance)) : (typeof recipe.chance === 'number' ? Math.max(0, Math.min(1, recipe.chance)) : 1)),
        });
    }
...
function getRecipesFromBlock(settings) {
    return {
...
const crusherDualComponent = {
    ...
    onTick(e, { params: settings }) {
        ...
        const recipePack = getRecipesFromBlock(settings);
        const customRecipeTable = loadCustomRecipeTable(machine.entity);
        const recipe = customRecipeTable?.[inputSlot.typeId] ?? resolveRecipe(recipePack, inputSlot.typeId);
        if (!recipe) {
...
const machineRecipesComponent = {
    onPlace(e, { params: settings }) {
        const entity = e?.block ? e.block.dimension.getEntitiesAtBlockLocation(e.block.location)[0] : undefined;
        if (entity) storeCustomRecipeTable(entity, settings);
        else system.run(() => {
            const retryEntity = e?.block ? e.block.dimension.getEntitiesAtBlockLocation(e.block.location)[0] : undefined;
            if (retryEntity) storeCustomRecipeTable(retryEntity, settings);
        });
    },
    onTick(e, { params: settings }) {
        const entity = e?.block ? e.block.dimension.getEntitiesAtBlockLocation(e.block.location)[0] : undefined;
...
DoriosAPI.register.blockComponent('machine_recipes', machineRecipesComponent);
